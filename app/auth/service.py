...
async def authenticate_user(username: str, password: str, telegram_user_id: int) -> bool:
    ...
    try:
        ...
        from app.services.deadline import auth_headers
        ...
        async with session.get(
            ...
            if resp.status == 200:
                ...
                from app.storage.user_settings import clear_auth_failure_notice

                clear_auth_suspension(username)
                await clear_auth_failure_notice(telegram_user_id)
                return True
            else:
