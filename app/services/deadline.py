...
def _extract_pool_names(payload: Any) -> List[str]:
    if isinstance(payload, list):
        return _normalize_name_list([str(item) for item in payload if item is not None])
    if isinstance(payload, dict):
        for key in ("Pools", "Pool", "Names", "Name"):
            value = payload.get(key)
            if isinstance(value, list):
                return _normalize_name_list([str(item) for item in value if item is not None])
            if isinstance(value, str):
                return _normalize_name_list([part for part in value.split(",") if part.strip()])
        # Some Deadline responses may map pool -> workers.
        if payload:
            return _normalize_name_list([str(k) for k in payload.keys()])
    if isinstance(payload, str):
        return _normalize_name_list([part for part in payload.split(",") if part.strip()])
    return []


def _extract_worker_names_for_pool(payload: Any, pool_name: str) -> List[str]:
    ...
    if isinstance(payload, dict):
        for key, value in payload.items():
            ...
            if key_lower == pool_name_lower or key_lower in {"workers", "slaves", "worker", "slave"}:
                if isinstance(value, list):
                    return _normalize_name_list([str(item) for item in value if item is not None])
                if isinstance(value, str):
                    return _normalize_name_list([part for part in value.split(",") if part.strip()])
        # fallback for structures like {"PoolName": ["worker1", ...]}
        for key, value in payload.items():
            if isinstance(value, list) and str(key).strip():
                return _normalize_name_list([str(item) for item in value if item is not None])
    return []


async def get_pool_names(login: str, password: str) -> List[str]:
    """Get all pool names."""
    try:
        session = await get_aiosession()
        auth = aiohttp.BasicAuth(login, password)
        async with session.get(
            f"{settings.deadline_api_url}/pools",
            auth=auth,
            ssl=settings.deadline_tls_verify,
        ) as resp:
            if resp.status != 200:
                response_text = await resp.text()
                logger.error(
                    "Failed to get pools: %s, response: %s",
                    resp.status,
                    response_text,
                )
                return []
            data = await resp.json()
            return _extract_pool_names(data)
    except Exception as exc:
        logger.error("Error getting pools: %s", exc)
        return []


async def get_pool_names_by_user_id(telegram_user_id: int) -> List[str]:
    """Get all pool names using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=[],
        operation_name="get pools",
        call=get_pool_names,
    )


async def get_workers_for_pool(
    login: str,
    password: str,
    pool_name: str,
) -> List[str]:
    """Get worker names for a pool."""
    ...
    try:
        ...
        async with session.get(
            ...
            if resp.status != 200:
                response_text = await resp.text()
                logger.error(
                    "Failed to get workers for pool %s: %s, response: %s",
                    normalized_pool,
                    resp.status,
                    response_text,
                )
                return []
            data = await resp.json()
            return _extract_worker_names_for_pool(data, normalized_pool)
    except Exception as exc:
        logger.error("Error getting workers for pool %s: %s", normalized_pool, exc)
        return []


async def get_workers_for_pool_by_user_id(
    telegram_user_id: int,
    pool_name: str,
) -> List[str]:
    """Get worker names for a pool using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=[],
        operation_name=f"get workers for pool ({pool_name})",
        call=lambda login, password: get_workers_for_pool(login, password, pool_name),
    )
...
async def add_pools(
    ...
    """Create new pools if missing."""
    normalized = _normalize_name_list(pools)
    if not normalized:
        return False
    try:
        session = await get_aiosession()
        auth = aiohttp.BasicAuth(login, password)
        payload = {"Pool": normalized}
        async with session.post(
            ...
            if resp.status not in (200, 201, 202, 204):
                logger.error(
                    "Failed to add pools %s: %s, response: %s",
                    normalized,
                    resp.status,
                    response_text,
                )
        ...
        logger.error("Error adding pools %s: %s", normalized, exc)
        return False


async def add_pools_by_user_id(telegram_user_id: int, pools: List[str]) -> bool:
    """Create pools using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=False,
        operation_name=f"add pools ({','.join(pools)})",
        call=lambda login, password: add_pools(login, password, pools),
    )


async def add_pools_to_workers(
    login: str,
    password: str,
    worker_names: List[str],
    pools: List[str],
    *,
    ...
    normalized_workers = _normalize_name_list(worker_names)
    normalized_pools = _normalize_name_list(pools)
    if not normalized_workers or not normalized_pools:
        return False
    payload: Dict[str, Any] = {
        "Slave": normalized_workers,
        "Pool": normalized_pools,
    }
    if overwrite:
    ...
    try:
        ...
        async with session.put(
            ...
            if resp.status not in (200, 201, 202, 204):
                logger.error(
                    "Failed to assign pools %s to workers %s: %s, response: %s",
                    normalized_pools,
                    normalized_workers,
                    resp.status,
                    response_text,
        ...
        logger.error(
            "Error assigning pools %s to workers %s: %s",
            normalized_pools,
            normalized_workers,
            exc,
        )
...
async def add_pools_to_workers_by_user_id(
    telegram_user_id: int,
    worker_names: List[str],
    pools: List[str],
    *,
    ...
    """Assign pools to workers using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=False,
        operation_name=f"assign pools ({','.join(pools)})",
        call=lambda login, password: add_pools_to_workers(
            login,
            password,
            worker_names,
            pools,
...
async def delete_pools(
    login: str,
    password: str,
    pools: List[str],
) -> bool:
    """Delete pools by name."""
    normalized = _normalize_name_list(pools)
    if not normalized:
        return False
    params = [("Pool", pool_name) for pool_name in normalized]
    try:
        session = await get_aiosession()
        auth = aiohttp.BasicAuth(login, password)
        async with session.delete(
            ...
            if resp.status not in (200, 201, 202, 204):
                logger.error(
                    "Failed to delete pools %s: %s, response: %s",
                    normalized,
                    resp.status,
                    response_text,
                )
        ...
        logger.error("Error deleting pools %s: %s", normalized, exc)
        return False


async def delete_pools_by_user_id(telegram_user_id: int, pools: List[str]) -> bool:
    """Delete pools using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=False,
        operation_name=f"delete pools ({','.join(pools)})",
        call=lambda login, password: delete_pools(login, password, pools),
    )


async def delete_pools_from_workers(
    login: str,
    password: str,
    pools: List[str],
    worker_names: List[str],
) -> bool:
    """Remove pools from specific workers."""
    normalized_pools = _normalize_name_list(pools)
    normalized_workers = _normalize_name_list(worker_names)
    if not normalized_pools or not normalized_workers:
        return False
    params = [("Pool", pool_name) for pool_name in normalized_pools]
    params.extend(("Slaves", worker_name) for worker_name in normalized_workers)
    try:
        session = await get_aiosession()
        auth = aiohttp.BasicAuth(login, password)
        async with session.delete(
            ...
            if resp.status not in (200, 201, 202, 204):
                logger.error(
                    "Failed to remove pools %s from workers %s: %s, response: %s",
                    normalized_pools,
                    normalized_workers,
                    resp.status,
                    response_text,
        ...
        logger.error(
            "Error removing pools %s from workers %s: %s",
            normalized_pools,
            normalized_workers,
            exc,
        )
...
async def delete_pools_from_workers_by_user_id(
    telegram_user_id: int,
    pools: List[str],
    worker_names: List[str],
) -> bool:
    """Remove pools from workers using Telegram user credentials."""
    return await _with_user_credentials(
        telegram_user_id,
        default=False,
        operation_name=f"remove pools ({','.join(pools)}) from workers",
        call=lambda login, password: delete_pools_from_workers(
            login,
            password,
            pools,
            worker_names,
